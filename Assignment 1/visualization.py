...
def update_satellite_pose(satellite_mesh, r_i, Theta_ib, degrees=False):
    ...
    if degrees:
        ...
        Theta_ib = [
            np.deg2rad(Theta_ib[0]),
            np.deg2rad(Theta_ib[1]),
            np.deg2rad(Theta_ib[2]),
        ]
    ...
    print(f"Rotation Angles for the satellite: {Theta_ib}")
    satellite_mesh["Body"].SetOrientation(np.rad2deg(Theta_ib))
    satellite_mesh["Solar Panels"][0].SetOrientation(np.rad2deg(Theta_ib))
    satellite_mesh["Solar Panels"][1].SetOrientation(np.rad2deg(Theta_ib))
    satellite_mesh["Scientific Instrument"].SetOrientation(np.rad2deg(Theta_ib))
...
def update_body_frame_pose(body_frame, r_i, Theta_ib):
    """
